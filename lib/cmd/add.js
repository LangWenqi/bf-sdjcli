...
const resolve = (result) => {
  const { tplName, gitUrl, options, description, } = result;
  // 避免重复添加
  if (!tpls[tplName]) {
    tpls[tplName] = {};
    tpls[tplName]['url'] = gitUrl.replace(/[\u0000-\u0019]/g, ''); // 过滤unicode字符
    tpls[tplName]['options'] = options;
...
module.exports = () => {
  co(function *() {
    ...
    const gitUrl = yield inputCallBack('Git https 或 ssh 链接: ');
    let options = yield prompt('Git options:（格式如下：sass,less,antd ');
		if (!options) {
			options = 'master'
		}
